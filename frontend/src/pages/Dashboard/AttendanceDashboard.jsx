...
import { generateEmployees,getTodayAttendance } from '../../utils/dataGenerator';
import { X, Eye, EyeOff, Sliders } from 'lucide-react';

const MemoizedSearchAndFilter = React.memo(SearchAndFilter);

const DashboardContent = () => {
  const [activeMenuItem, setActiveMenuItem] = useState('Dashboard'); // Main navigation active item
...
useEffect(() => {
  if (employees.length > 0) {
    const stats = getTodayAttendance(employees);
    setTodayStats(stats);
    console.log('Today Attendance:', stats); // Debug ke liye
  }
}, [employees]);
  // ESC key handler
  useEffect(() => {
  ...
  return (
    <div className={`h-screen flex ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      {/* Sidebar - now receives activeMainMenuItem */}
      <Sidebar 
        onOpenSettings={openSettingsPopup} 
        activeMainMenuItem={activeMenuItem}
      />
          ...
          <div className={getSpacingClasses()}>
            ...
            {activeMenuItem === 'Dashboard' && (
              <>
                ...
                {componentVisibility.statsCards && (
                  <div className="mb-4 sm:mb-5 lg:mb-6">
                    <StatsSection stats={stats}   todayData={todayStats} darkMode={darkMode} />
                  </div>
                )}
            ...
            {activeMenuItem === 'Leave' && (
              <div className="space-y-6">
                ...
                {/* Add Leave-specific content here */}
                <div className={`p-6 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow`}>
                  <p className={`${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
