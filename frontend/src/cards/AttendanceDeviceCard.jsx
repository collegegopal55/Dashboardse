...
//   // Get device statistics (simplified - only total today and last punch)
...
//       {/* Device Statistics Section - Simplified */}
//       {showDeviceStats ? (
...
//                 {/* Stats Grid - Simplified to only show Total Today and Last Punch */}
...
import { 
  Clock, MoreVertical, Fingerprint, Smartphone, UserCheck,
  Monitor, Wifi, WifiOff, Battery, BatteryCharging, LogIn, LogOut
} from 'lucide-react';
import { ATTENDANCE_TYPES, EMPLOYEE_STATUS } from '../constants/index';
...
const AttendanceDeviceCard = ({ 
  ...
  // Effect to handle blinking for new biometric punches
  useEffect(() => {
    ...
    punches.forEach(punch => {
      ...
      if (punch.type === ATTENDANCE_TYPES.BIOMETRIC && 
          punch.timestamp && 
  ...
  const generateRecentPunches = (employees, count = 20) => {
    ...
    const punchTypes = [ATTENDANCE_TYPES.BIOMETRIC, ATTENDANCE_TYPES.MOBILE, ATTENDANCE_TYPES.MANUAL];
    const devices_list = ['Main Gate', 'Side Gate', 'Office Entry'];
    const now = Date.now();
  ...
  const shouldBlink = (punch) => {
    return punch.type === ATTENDANCE_TYPES.BIOMETRIC && blinkingDevices[punch.device] ? 'animate-pulse' : '';
  };

  // Check if device should be shown (sirf biometric punches mein)
  const shouldShowDevice = (punch) => {
    return punch.type === ATTENDANCE_TYPES.BIOMETRIC;
  };
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center flex-wrap gap-x-3 gap-y-1 mb-1">
                      ...
                      <div className={`flex items-center space-x-1 text-xs px-1.5 py-0.5 rounded ${
                        ...
                        {getPunchTypeIcon(punch.type)}
                        <span className="capitalize">{punch.typeName || punch.type}</span>
                      </div>
