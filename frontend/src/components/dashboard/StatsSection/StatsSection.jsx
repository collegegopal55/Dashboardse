...
// components/dashboard/StatsSection/StatsSection.js mein ye change karein
...
const StatsSection = ({ stats, todayData, darkMode }) => {
  
  // Priority: todayData -> stats -> default values
  const safeStats = todayData || stats || {
    activeUsers: 0,
    todayAttendance: {
      onTime: 0,
      late: 0,
  ...
  const attendanceStats = [
    { 
      ...
      icon: Users, 
      description: 'Total active employees today' 
    },
    { 
      name: 'ON TIME', 
      value: safeStats.todayAttendance?.onTime || 0, 
      color: COLORS.success, 
      icon: CheckCircle, 
      description: 'Employees who arrived on time' 
    },
    { 
      name: 'LATE', 
      value: safeStats.todayAttendance?.late || 0, 
    ...
    { 
      name: 'ABSENT', 
      value: safeStats.todayAttendance?.absent || 0, 
      color: COLORS.danger, 
      icon: XCircle, 
    ...
    { 
      name: 'LEAVE', 
      value: safeStats.todayAttendance?.leave || 0, 
      color: COLORS.purple, 
      icon: Calendar, 
    ...
    { 
      name: 'TOUR', 
      value: safeStats.todayAttendance?.tour || 0, 
      color: COLORS.pink, 
      icon: MapPin, 
      description: 'Employees on tour' 
    },
    { 
      name: 'HOLIDAY', 
      value: safeStats.todayAttendance?.holiday || 0, 
      color: COLORS.teal, 
      icon: Clock, 
    ...
    { 
      name: 'WEEKLY OFF', 
      value: safeStats.todayAttendance?.weeklyOff || 0, 
      color: COLORS.gray, 
      icon: Home, 
      description: 'Weekly off employees' 
    },
  ];
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-8 gap-4 mb-6">
      {attendanceStats.map((stat, index) => (
        <StatCard 
          ...
          activeUsers={safeStats.activeUsers} 
          darkMode={darkMode}
          showTrend={stat.name !== 'ACTIVE USER'}
        />
      ))}
    </div>
  );
